...
IF OBJECT_ID('ClinicaTurbia.TOP5BONOSVENCIDOS', 'P') IS NOT NULL
	DROP PROCEDURE ClinicaTurbia.TOP5BONOSVENCIDOS
GO
...
IF OBJECT_ID('ClinicaTurbia.TOP5ESPECIALIDADES', 'P') IS NOT NULL
	DROP PROCEDURE ClinicaTurbia.TOP5ESPECIALIDADES
GO
...
CREATE PROCEDURE ClinicaTurbia.REGISTRAR_LLEGADA
	...
	UPDATE ClinicaTurbia.BonoConsulta
		SET BONOCON_NUMERO_CONSULTA =
		((SELECT MAX(BONOCON_NUMERO_CONSULTA) FROM ClinicaTurbia.BonoConsulta WHERE BONOCON_AFILIADO = @dniAfi) + 1)
...
CREATE PROCEDURE ClinicaTurbia.TOP5ESPECIALIDADES AS
	SELECT TOP 5 ESP_DESCRIPCION, COUNT(*) AS 'CANTIDAD DE CANCELACIONES'
		...
		ORDER BY 'CANTIDAD DE CANCELACIONES';
GO

CREATE PROCEDURE ClinicaTurbia.TOP5BONOSVENCIDOS 
	(@fechaActual datetime) AS
SELECT TOP 5 (PAC_NOMBRE + ' ' + PAC_APELLIDO) AS NOMBRE, BONOFAR_AFILIADO AS DOCUMENTO, COUNT(*) AS 'BONOS VENCIDOS'
	...
	AND PAC_NUMDOC = BONOFAR_AFILIADO
	GROUP BY BONOFAR_AFILIADO, (PAC_NOMBRE + ' ' + PAC_APELLIDO)
	ORDER BY 'BONOS VENCIDOS'
GO
