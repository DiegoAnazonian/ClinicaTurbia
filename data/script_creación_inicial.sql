...
IF OBJECT_ID('ClinicaTurbia.TOP5_BONOS_AJENOS','P') IS NOT NULL
	DROP PROCEDURE ClinicaTurbia.TOP5_BONOS_AJENOS
GO
...
CREATE PROCEDURE ClinicaTurbia.GENERAR_RECETA
	(@bono numeric(18,0), @medi nvarchar(255), @cant int, @fecha datetime, @pac numeric(18,0)) AS
...
SELECT TOP 5 (PAC_NOMBRE + ' ' + PAC_APELLIDO) AS NOMBRE, BONOFAR_AFILIADO_COMPRO AS DOCUMENTO, COUNT(*) AS 'BONOS VENCIDOS'
	...
	GROUP BY BONOFAR_AFILIADO_COMPRO, (PAC_NOMBRE + ' ' + PAC_APELLIDO)
...
CREATE PROCEDURE ClinicaTurbia.TOP5_BONOS_AJENOS AS
	SELECT AFILIADO, SUM(CANT) AS CANT FROM 
		...
		UNION ALL
		SELECT BONOFAR_AFILIADO_USO AS AFILIADO, COUNT(*) AS CANT
